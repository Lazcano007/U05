...
export const getJournals =  async (req: Request, res: Response) => {
    ...
    res.status(200).json(journals);
        ...
        res.status(500).json({message: "There's been an error fetching the journal", error: error.message});
    }
};

// Detta uppdaterar DATA i DB (i detta fall journalen)
export const getJournalsForPet = async (req: Request, res: Response): Promise<void> =>  {
    try {
      const { petId } = req.params;
      const journals = await Journals.find({ petId });
      if (!journals || journals.length === 0) {
       res.status(404).json({ message: "There's no journals found for this pet" });
       return;
      }
      res.status(200).json(journals);
      return;
...
export const createJournal = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!petId ||!description ||!treatment ||!dateDiagnosis ||!status) {
            res.status(400).json({message: "The PetId, description, treatment, dateDiagnosis, and status are required."});
            return;
        }
        const newJournal = new Journals({petId, description, treatment, dateDiagnosis, status});
...
export const updateJournal = async (req: Request, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        const updatedJournal = await journalModel.findByIdAndUpdate( id, req.body,   {new: true});  
        if (!updatedJournal) {                                                                                           
        res.status(404).json({message: "This journal failed to upadte"});    
        return;
    }
    res.json({message: "Your journal has been successfully updated", journal: updatedJournal});
    } catch (error:any) {
        res.status(500).json({message: "There's been a server error"})
    }
}
