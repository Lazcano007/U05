...
export const getUserPets = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "There's been an error getting your pets" });
  }
};
...
export const createPet = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (!name || !species || !breed || age === undefined || !owner) {
      res
        .status(400)
        .json({ message: "Name, species, breed, age and owner are required." });
      return;
    }
...
export const updatePet = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const updatedPet = await petModel.findByIdAndUpdate(id, req.body, {
      new: true,
    });
    if (!updatedPet) {
      res.status(404).json({ message: "This pet is not found" });
    ...
    res.json({
      message: "Your pet has been successfully updated",
      pet: updatedPet,
    });
  } catch (error: any) {
    res.status(500).json({ message: "There's been a server error" });
